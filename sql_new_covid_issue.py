...
def add_Info(data, cur, conn):
    ...
    for (id_num, date_id, year, month, state, total_cases, confirmed, hospitalized, deaths, dailychg_cases) in data:
        ...
        if  id <= id_num < stop: 
            cur.execute('INSERT OR IGNORE INTO Covid(id_num, date_id, year, month, state, total_cases, confirmed, hospitalized, deaths, dailychg_cases) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (id_num, date_id, year, month, state, total_cases, confirmed, hospitalized, deaths, dailychg_cases))
            conn.commit()
            if  id_num == data[-1][0]:
